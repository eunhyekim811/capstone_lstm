...
- 재학습 시 새로 쌓이는 데이터로만 학습할 수 있도록 코드 수정 (진행중)

    - 현재로서는 기존 데이터들은 반복적으로 재학습하는 형태이다...

    - 맨처음 사용자 등록 시 테이블에 저장하는 과정 필요

    - 학습 데이터 로드 시 테이블 참고해 필요한 데이터만 조회하도록 수정

- 샘플 데이터 수집 (구현 완료)

    - 테스트 필요

    - 평균 cpu, disk 임계값 기준 계산 수정 필요
